...
# This configuration expands on the previous entries.

# load `zcalc` and `zmv` helpers
...
setopt RC_QUOTES        # use single quotes within a single-quoted string
setopt CSH_NULL_GLOB    # error message when pattern fails, behave like null_glob otherwise
...
# ** Source helper config files **
#   Remember to update paths and filenames accordingly

# load key-bindings from Chapter 3
[[ -f ~/keybinds.zsh ]] && source ~/keybinds.zsh

# load completion config from Chapter 5
[[ -f ~/completion.zsh ]] && source ~/completion.zsh

# load helper functions from Chapter 6
